...
def configure_aws_credentials():
    ...
    credentials are already configured. If not, it prompts the
    user to enter their AWS credentials and optionally saves
    them to a configuration file for future use.
    """
    try:
        boto3.client("sts").get_caller_identity()
        logger.info("AWS credentials are already configured.")
    except (NoCredentialsError, PartialCredentialsError):
        logger.info("AWS credentials not found. Please enter your AWS credentials.")
        ...
        aws_session_token = getpass(
            "AWS Session Token (leave empty if not applicable): "
        ).strip()

        if not aws_access_key_id or not aws_secret_access_key:
            logger.error("AWS Access Key ID and Secret Access Key are required.")
            return
        ...
        save_credentials = (
            input("Do you want to save these credentials for future use? (y/n): ")
            .strip()
            .lower()
        )
...
def save_credentials_to_file(
    ...
    try:
        ...
        logger.info("Credentials saved to %s", config_file)
    except IOError as error:
        logger.error("Error saving credentials to file %s: %s", config_file, error)

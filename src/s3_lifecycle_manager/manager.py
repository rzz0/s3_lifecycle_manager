...
and extracting the lifecycle rules to save in a CSV file. It includes information
about buckets that do not have lifecycle rules.
...
class S3LifecycleManager:
    ...
    def get_lifecycle_policy(self, bucket_name: str) -> List[Dict[str, Any]]:
        ...
        :returns: List of lifecycle rules.
        :rtype: List[Dict[str, Any]]
        """
        try:
            ...
            if error.response['Error']['Code'] == 'NoSuchLifecycleConfiguration':
                ...
                self.logger.warning(
                    "Unable to get the lifecycle policy for bucket %s: %s", bucket_name, error)
            return []
    ...
    def analyze_rule(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        ...
        noncurrent_expiration = rule.get(
            'NoncurrentVersionExpiration', {}).get('NoncurrentDays', 'N/A')
        abort_multipart = rule.get(
            'AbortIncompleteMultipartUpload', {}).get('DaysAfterInitiation', 'N/A')
    ...
    def save_policies_csv(self, filename: str) -> None:
        ...
        try:
            fieldnames = [
                'Bucket', 'Status', 'ID', 'Prefix', 'Transitions',
                'ExpirationDays', 'NoncurrentVersionTransitions',
            ...
            self.logger.info(
                "Policies successfully saved to %s", filename)
